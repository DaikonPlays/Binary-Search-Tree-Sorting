 ...
 * 
 * iterator for a binary search tree. Used
 * to help run through binary search tree
 */
import java.util.Iterator;
import java.util.NoSuchElementException;
/**
 * Iterator class that implements a BST iterator. 
 * It contains methods to help with node traversal.
 * Instance variables:
 * next, next node to visit
 * lastVisited, last node visited 
 */
public class MyBSTIterator<K extends Comparable<K>, V> extends MyBST<K, V> {
    abstract class MyBSTNodeIterator<T> implements Iterator<T> {
         ...
         */
        public void remove() {
            //checks if there was no node visited
            if (lastVisited == null) {
            ...
            if (lastVisited.getRight() != null &&
                ...
                //moves next back if there are children
                next = lastVisited;
            }
            //removes the previous current node
            MyBSTIterator.this.remove(lastVisited.getKey());
            //sets lastVisited to null
            lastVisited = null;
        }
    }
